...
from .forms import SearchForm, InstitutionForm, KeywordForm
...
# if this is a POST request we need to process the form data
    ...
    # if a GET (or any other method) we'll create a blank form
        ...
        if request.GET.dict():
            ...
            projects_list = projects_list.filter(year__gte = 2018)
            projects_list = projects_list.exclude(awards = "-")
            projects_list = projects_list.exclude(section = 'High School')
...
# if this is a POST request we need to process the form data
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            query_string = '?' + '&'.join(query_filters)
            return redirect(reverse('keyword') + query_string)
    ...
    # if a GET (or any other method) we'll create a blank form
        ...
        if request.GET.get('keyword'):
            ...
            projects_list = projects_list.filter(year__gte = 2020)


        form = KeywordForm(initial=request.GET)
        context = {'projects_list': projects_list, 'form':form}
